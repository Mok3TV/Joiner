...
class utils():
    ...
    def getAppVersion():
        resp = requests.get(
            "https://discord.com/api/downloads/distributions/app/installers/latest?channel=stable&platform=win&arch=x86").text

        return resp.split("/distro/app/stable/win/x86/")[1].split("/DiscordSetup.exe")[0]

    def getBuildNumber():
        resp = requests.get(
            f"https://discord.com/assets/{utils.getLatestJS()}")
