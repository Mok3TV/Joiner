from modules.base import AbsoluteBase
...
class creator(AbsoluteBase):
    ...
    def register(self) -> None:
        ...
        if "token" in resp.text:
            ...
            self.logger.valid(
                title="CREATED", data=f"{self.token.split('.')[0]}.{'*'*24}")
            with open("output/tokens.txt", "a") as f:
                f.write(f"{self.token}\n")
                f.close()
            threading.Thread(target=self.websocketHandler,
                             args=(self.token,)).start()
            self.session.headers["authorization"] = self.token
        elif "retry_after" in resp.text:
            delay = resp.json()["retry_after"]
            self.logger.warn(title="RATELIMIT",
                             data=f"Waiting for {delay}s...")
            time.sleep(delay)
        elif "invalid-response" or "invalid-input-response" in resp.text:
            self.logger.warn(
                title="ERROR", data=f"Captcha error: {self.captchaSolution.split('.')[0]}")
    ...
    def websocketHandler(self, token: str):
        while True:
            try:
                ...
                ws.send(thing)
                heartbeat = json.loads(ws.recv()).get(
